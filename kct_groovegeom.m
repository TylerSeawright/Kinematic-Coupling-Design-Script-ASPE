function [T_GC_mfgG, T_V11_GC, T_V12_GC, T_V21_GC, T_V22_GC, T_V31_GC,T_V32_GC] = kct_groovegeom(Pb, Height_V_mfgG, Or, Halfa, Db, d_pin, v_reo)
...
% DeltaZ_mfgG: Y Offset of the vee bottom, is a 3*1
...
DeltaX_mfgG = d_pin(1,1:3);
DeltaY_mfgG = d_pin(2,1:3);
DeltaZ_mfgG = d_pin(3,1:3);
...
for i = 1:2:6
    j = (i+1)/2;
    Dist_CV(j) = (Db(j)/2) / sin((Halfa(i) +Halfa(i+1)) / 2); % Distance between vertex and center of the ball
    X_G_mfgG(j) = X_V_mfgG(j) - Dist_CV(j) *sin((Halfa(i) - Halfa(i+1)) /2) *sin(Or(j));
    Y_G_mfgG(j) = Y_V_mfgG(j) + Dist_CV(j) *sin((Halfa(i) - Halfa(i+1)) /2) *cos(Or(j));
    Z_G_mfgG(j) = Height_V_mfgG(j) + Dist_CV(j) *cos((Halfa(i) - Halfa(i+1)) /2);
end

function f = kc_plot_FBD(kc, tg, plot_title)
    ...
    szFL = size(FL);
    for i = 1:szFL(2)
        FL_unit(:,i) = FL(:,i)./norm(FL(:,i));
        FL_scale{i} = -uvec_scale * max_dist .* [[0,0,0]', FL_unit(:,i)];
        FL_scale{i} = FL_scale{i} + FL_loc(:,i);
    end
    for i = 1:3
    ...
    % Csys Plot Data
    % BC_Csys = uvec_scale * max_dist * data_transform(inv(kc.T_input), COORD()')' + C;
    ...
    plot3(C(1),C(2), C(3),'og', 'MarkerSize', 15);plot3(C(1),C(2), C(3),'.g', 'MarkerSize', 15);
    % plot3(BC_Csys(1,:),BC_Csys(2,:),BC_Csys(3,:),'-g', 'LineWidth', 3)
    text(C(1) - bl_offset, C(2) - bl_offset, C(3), "C", 'color', 'k')

    % Plot Force Input Location
    for i = 1:szFL(2)
        plot3(FL_loc(1,i),FL_loc(2,i), FL_loc(3,i), "*r", 'MarkerSize', 8, 'LineWidth', 2);
        plot3(FL_scale{i}(1,:), FL_scale{i}(2,:), FL_scale{i}(3,:), "-r", 'LineWidth', 2);
        text(FL_loc(1,i) + bl_offset, FL_loc(2,i) + bl_offset, FL_loc(3,i) + bl_offset, sprintf('FL%d: %.1f N',i,norm(FL(:,i))), 'color', 'r')
    end
    ...
    if (tg.F_P_is_equal == 0 || tg.F_P_is_equal == 1)
        ...
        text(PL_loc(1,i) + bl_offset, PL_loc(2,i) + bl_offset, PL_loc(3,i) + bl_offset, sprintf('PL%d: %.1f N',i,norm(PL(1:3,i))), 'color', 'k')   
        end
    elseif (tg.F_P_is_equal == 2)
        PLx = PL_scale{1}(1,:); PLy = PL_scale{1}(2,:); PLz = PL_scale{1}(3,:);
        plot3(PLx,PLy, PLz, "-k", 'MarkerSize', 8, 'LineWidth', 2);
        plot3(PL_loc(1),PL_loc(2), PL_loc(3), "*k", 'MarkerSize', 8, 'LineWidth', 2);
        text(PL_loc(1) + bl_offset, PL_loc(2) + bl_offset, PL_loc(3) + bl_offset, sprintf('PL: %.1f N',norm(PL(1:3,1))), 'color', 'k')   
    end 
    ...
    for i = 1:size(kc.poi,1)
        ...
        text(kc.poi(i,1) + bl_offset/3, kc.poi(i,2) + bl_offset/3, kc.poi(i,3) + bl_offset/3, sprintf('POI: %d ', i), 'color', 'k')
    end
    %% Figure Format
    axis square
    axis equal
